...
dispWin_t dispWin = { .x1 = 0, .y1 = 0, .x2 = DEFAULT_TFT_DISPLAY_WIDTH, .y2 =
		DEFAULT_TFT_DISPLAY_HEIGHT, };

Font cfont = { .font = tft_DefaultFont, .x_size = 0, .y_size = 0x0B,
		.offset = 0, .numchars = 95, .bitmap = 1, };
...
static void _drawPixel(int16_t x, int16_t y, color_t color, uint8_t sel) {
	...
	if ((x < dispWin.x1) || (y < dispWin.y1) || (x > dispWin.x2)
			|| (y > dispWin.y2))
		return;
...
//----------------------------------------------------------------------------------
static void _drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
		color_t color) {
	if (x0 == x1) {
		if (y0 <= y1)
...
void TFT_drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, color_t color) {
	_drawLine(x0 + dispWin.x1, y0 + dispWin.y1, x1 + dispWin.x1,
			y1 + dispWin.y1, color);
}
...
void TFT_fillScreen(color_t color) {
	TFT_pushColorRep(0, 0, _width - 1, _height - 1, color,
			(uint32_t)(_height * _width));
}

//==================================
void TFT_fillWindow(color_t color) {
	TFT_pushColorRep(dispWin.x1, dispWin.y1, dispWin.x2, dispWin.y2, color,
			(uint32_t)(
					(dispWin.x2 - dispWin.x1 + 1)
							* (dispWin.y2 - dispWin.y1 + 1)));
}
...
//-----------------------------------------------------------------------------------
static void _drawRect(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h,
		color_t color) {
...
//=============================================================================================
void TFT_drawRoundRect(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t r,
		color_t color) {
	x += dispWin.x1;
	y += dispWin.y1;
...
//=============================================================================================
void TFT_fillRoundRect(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t r,
		color_t color) {
	x += dispWin.x1;
	y += dispWin.y1;
...
//-----------------------------------------------------------------------------------------------
static void _drawLineByAngle(int16_t x, int16_t y, int16_t angle,
		uint16_t length, color_t color) {
	_drawLine(x, y, x + length * cos((angle + _angleOffset) * DEG_TO_RAD),
			y + length * sin((angle + _angleOffset) * DEG_TO_RAD), color);
}

//---------------------------------------------------------------------------------------------------------------
static void _DrawLineByAngle(int16_t x, int16_t y, int16_t angle,
		uint16_t start, uint16_t length, color_t color) {
	_drawLine(x + start * cos((angle + _angleOffset) * DEG_TO_RAD),
			y + start * sin((angle + _angleOffset) * DEG_TO_RAD),
			x + (start + length) * cos((angle + _angleOffset) * DEG_TO_RAD),
			y + (start + length) * sin((angle + _angleOffset) * DEG_TO_RAD),
			color);
}

//===========================================================================================================
void TFT_drawLineByAngle(uint16_t x, uint16_t y, uint16_t start, uint16_t len,
		uint16_t angle, color_t color) {
	x += dispWin.x1;
	y += dispWin.y1;
...
// Draw a triangle
//--------------------------------------------------------------------------------------------------------------------
static void _drawTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
...
// Fill a triangle
//--------------------------------------------------------------------------------------------------------------------
static void _fillTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
		uint16_t x2, uint16_t y2, color_t color) {
	int16_t a, b, y, last;
...
void TFT_fillTriangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
		uint16_t x2, uint16_t y2, color_t color) {
	_fillTriangle(x0 + dispWin.x1, y0 + dispWin.y1, x1 + dispWin.x1,
			y1 + dispWin.y1, x2 + dispWin.x1, y2 + dispWin.y1, color);
}
...
//----------------------------------------------------------------------------------------------------------------
static void _draw_ellipse_section(uint16_t x, uint16_t y, uint16_t x0,
		uint16_t y0, color_t color, uint8_t option) {
	disp_select();
	// upper right
...
//-----------------------------------------------------------------------------------------------------------------------
static void _draw_filled_ellipse_section(uint16_t x, uint16_t y, uint16_t x0,
		uint16_t y0, color_t color, uint8_t option) {
	// upper right
	if (option & TFT_ELLIPSE_UPPER_RIGHT)
...
//---------------------------------------------------------------------------------------------------------------------------------
static void _fillArcOffsetted(uint16_t cx, uint16_t cy, uint16_t radius,
		uint16_t thickness, float start, float end, color_t color) {
	//float sslope = (float)cos_lookup(start) / (float)sin_lookup(start);
	//float eslope = (float)cos_lookup(end) / (float)sin_lookup(end);
	float sslope = (cos(start / _arcAngleMax * 2 * PI) * _arcAngleMax)
...
//===========================================================================================================================
void TFT_drawArc(uint16_t cx, uint16_t cy, uint16_t r, uint16_t th, float start,
		float end, color_t color, color_t fillcolor) {
	cx += dispWin.x1;
	cy += dispWin.y1;
...
//=============================================================================================================
void TFT_drawPolygon(int cx, int cy, int sides, int diameter, color_t color,
		color_t fill, int rot, uint8_t th) {
	cx += dispWin.x1;
	cy += dispWin.y1;
	...
	if (f) {
		for (int idx = 0; idx < sides; idx++) {
			if ((idx + 1) < sides)
				_fillTriangle(cx, cy, Xpoints[idx], Ypoints[idx],
						Xpoints[idx + 1], Ypoints[idx + 1], fill);
			else
				_fillTriangle(cx, cy, Xpoints[idx], Ypoints[idx], Xpoints[0],
						Ypoints[0], fill);
		}
	}
	...
	if (th) {
		for (int n = 0; n < th; n++) {
			if (n > 0) {
				for (int idx = 0; idx < sides; idx++) {
					Xpoints[idx] = cx
							+ sin((float) (idx * rads + deg) * deg_to_rad)
									* (diameter - n);
					Ypoints[idx] = cy
							+ cos((float) (idx * rads + deg) * deg_to_rad)
									* (diameter - n);
				}
			}
			for (int idx = 0; idx < sides; idx++) {
				if ((idx + 1) < sides)
					_drawLine(Xpoints[idx], Ypoints[idx], Xpoints[idx + 1],
							Ypoints[idx + 1], color); // draw the lines
				else
					_drawLine(Xpoints[idx], Ypoints[idx], Xpoints[0],
							Ypoints[0], color); // finishes the last line to close up the polygon.
			}
		}
	}
}
...
static int load_file_font(const char *fontfile, int info) {
	...
	if (size != (read - 8)) {
		sprintf(err_msg, "Font size error: found %d expected %d)", size,
				(read - 8));
		err = 7;
		goto exit;
	}
...
int compile_font_file(char *fontfile, uint8_t dbg) {
	...
	*fend = '\0';
	...
	error: sprintf(err_msg, "error writing to destination file");
	err = 9;
...
static int printProportionalChar(int x, int y) {
	...
	char_width = (
			(fontChar.width > fontChar.xDelta) ?
					fontChar.width : fontChar.xDelta);
		...
		if (color_line) {
			...
			for (j = 0; j < fontChar.height; j++) {
				for (i = 0; i < fontChar.width; i++) {
					...
					if ((ch & mask) != 0) {
						// visible pixel
						bufPos = ((j + fontChar.adjYOffset) * char_width)
								+ (fontChar.xOffset + i);  // bufY + bufX
			...
			disp_select();
			send_data(x, y, x + char_width - 1, y + cfont.y_size - 1, len,
					color_line);
			disp_deselect();
			free(color_line);
		...
		if (color_line) {
			...
			for (j = 0; j < cfont.y_size; j++) {
				for (k = 0; k < fz; k++) {
					...
					for (i = 0; i < 8; i++) {
						if ((ch & mask) != 0)
							color_line[(j * cfont.x_size) + (i + (k * 8))] =
									_fg;
						mask >>= 1;
					}
				}
			...
			// send to display in one transaction
			disp_select();
			send_data(x, y, x + cfont.x_size - 1, y + cfont.y_size - 1, len,
					color_line);
			disp_deselect();
			free(color_line);
	...
	if (cfont.bitmap == 2)
		...
		while (*tempStrptr != 0) {
			if (getCharPtr(*tempStrptr++)) {
				strWidth += ((
						(fontChar.width > fontChar.xDelta) ?
								fontChar.width : fontChar.xDelta) + 1);
			}
		}
		strWidth--;
	}
 ...
 * 		     4
 */
static const uint16_t font_bcd[] = { 0x200, // 0010 0000 0000  // -
...
static void barVert(int16_t x, int16_t y, int16_t w, int16_t l, color_t color,
		color_t outline) {
	_fillTriangle(x + 1, y + 2 * w, x + w, y + w + 1, x + 2 * w - 1, y + 2 * w,
			color);
	_fillTriangle(x + 1, y + 2 * w + l + 1, x + w, y + 3 * w + l, x + 2 * w - 1,
			y + 2 * w + l + 1, color);
	_fillRect(x, y + 2 * w + 1, 2 * w + 1, l, color);
	if (cfont.offset) {
		_drawTriangle(x + 1, y + 2 * w, x + w, y + w + 1, x + 2 * w - 1,
				y + 2 * w, outline);
		_drawTriangle(x + 1, y + 2 * w + l + 1, x + w, y + 3 * w + l,
				x + 2 * w - 1, y + 2 * w + l + 1, outline);
...
static void barHor(int16_t x, int16_t y, int16_t w, int16_t l, color_t color,
		color_t outline) {
	_fillTriangle(x + 2 * w, y + 2 * w - 1, x + w + 1, y + w, x + 2 * w, y + 1,
			color);
	_fillTriangle(x + 2 * w + l + 1, y + 2 * w - 1, x + 3 * w + l, y + w,
			x + 2 * w + l + 1, y + 1, color);
	_fillRect(x + 2 * w + 1, y, l, 2 * w + 1, color);
	if (cfont.offset) {
		_drawTriangle(x + 2 * w, y + 2 * w - 1, x + w + 1, y + w, x + 2 * w,
				y + 1, outline);
		_drawTriangle(x + 2 * w + l + 1, y + 2 * w - 1, x + 3 * w + l, y + w,
				x + 2 * w + l + 1, y + 1, outline);
		_drawRect(x + 2 * w + 1, y, l, 2 * w + 1, outline);
	}
}

//--------------------------------------------------------------------------------------------
static void _draw7seg(int16_t x, int16_t y, int8_t num, int16_t w, int16_t l,
		color_t color) {
	/* TODO: clipping */
	...
	if (!(c & 0x800)) {
		// up middle point
		_fillRect(x + (d / 2), y + (2 * w) + 1 + (l / 2), 2 * w + 1, l / 2,
				_bg);
		if (cfont.offset)
			_drawRect(x + (d / 2), y + (2 * w) + 1 + (l / 2), 2 * w + 1, l / 2,
					_bg);
	}
	if (!(c & 0x200)) {
		// middle, minus
	...
	if (c & 0x080) {
		...
		if (cfont.offset)
			_drawRect(x + (d / 2), y + 2 * d, 2 * w + 1, 2 * w + 1,
					cfont.color);
	}
	if (c & 0x100) {
		// down middle point
		...
		if (cfont.offset)
			_drawRect(x + (d / 2), y + d + 2 * w + 1, 2 * w + 1, l / 2,
					cfont.color);
	}
	if (c & 0x800) {
		// up middle point
		...
		if (ch == 0x0D) { // === '\r', erase to eol ====
			...
			if (cfont.x_size == 0) {
				...
				if (cfont.bitmap == 1) {
					...
					// == 7-segment font ==
					_draw7seg(TFT_X, TFT_Y, ch, cfont.y_size, cfont.x_size,
							_fg);
					TFT_X += (tmpw + 2);
				}
			}
...
//---------------------
static UINT tjd_input(JDEC *jd,		// Decompression object
...
//-------------------------
static UINT tjd_buf_input(JDEC *jd,		// Decompression object
		BYTE *buff,		// Pointer to the read buffer (NULL:skip)
		UINT nd			// Number of bytes to read/skip from input stream
		) {
	// Device identifier for the session (5th argument of jd_prepare function)
...
//----------------------
static UINT tjd_output(JDEC *jd,	// Decompression object of current session
	...
	if ((len > 0) && (len <= JPG_IMAGE_LINE_BUF_SIZE)) {
		...
		wait_trans_finish(1);
		send_data(dleft, dtop, dright, dbottom, len,
				dev->linbuf[dev->linbuf_idx]);
...
// X & Y can be < 0 !
//==================================================================================
void TFT_jpg_image(int x, int y, uint8_t scale, char *fname, uint8_t *buf,
		int size) {
	JPGIODEV dev;
	struct stat sb;
	...
	if (work) {
		...
		if (rc == JDR_OK) {
			if (x == CENTER)
				x = ((dispWin.x2 - dispWin.x1 + 1 - (int) (jd.width >> scale))
						/ 2) + dispWin.x1;
			else if (x == RIGHT)
				x = dispWin.x2 + 1 - (int) (jd.width >> scale);

			if (y == CENTER)
				y = ((dispWin.y2 - dispWin.y1 + 1 - (int) (jd.height >> scale))
						/ 2) + dispWin.y1;
			else if (y == BOTTOM)
			...
			dev.linbuf[0] = heap_caps_malloc(JPG_IMAGE_LINE_BUF_SIZE * 3,
					MALLOC_CAP_DMA);
			...
			dev.linbuf[1] = heap_caps_malloc(JPG_IMAGE_LINE_BUF_SIZE * 3,
					MALLOC_CAP_DMA);
			...
			if (image_debug)
				printf(
						"Jpg size: %dx%d, position; %d,%d, scale: %d, bytes used: %d\r\n",
...
//====================================================================================
int TFT_bmp_image(int x, int y, uint8_t scale, char *fname, uint8_t *imgbuf,
		int size) {
	FILE *fhndl = NULL;
	struct stat sb;
	int i, err = 0;
	...
	if ((x < ((dispWin.x2 + 1) * -1)) || (x > (dispWin.x2 + 1))
			|| (y < ((dispWin.y2 + 1) * -1)) || (y > (dispWin.y2 + 1))) {
	...
	if ((img_xlen < 8) || (img_ylen < 8) || (img_xstart >= (img_xsize - 2))
			|| ((img_ysize - img_ystart) < 2)) {
	...
	if (image_debug)
		printf(
				...
				img_xsize, img_ysize, scale_pix, img_xlen, img_ylen, img_xstart,
				img_ystart, disp_xstart, disp_ystart, img_xsize * 3,
				((scale) ? (rd_len * scale_pix) : 0));
	...
	while ((disp_yend >= disp_ystart) && ((img_pos + (img_xsize * 3)) <= size)) {
		...
		if (scale == 0) {
			...
			if (fhndl) {
				...
				if (i != (img_xsize * 3)) {
					sprintf(err_buf, "file read at %d (%d<>%d)", img_pos, i,
							img_xsize * 3);
					err = -16;
					goto exit1;
				}
			...
			if (img_xstart > 0)
				memmove(line_buf[lb_idx], line_buf[lb_idx] + (img_xstart * 3),
						rd_len);
			...
			for (scan_lines = 0; scan_lines < scale_pix; scan_lines++) {
				...
				memcpy(scale_buf + (rd_len * scan_lines),
						line_buf[lb_idx] + img_xstart, rd_len);
			}
		...
		wait_trans_finish(1);
		send_data(disp_xstart, disp_yend, disp_xend, disp_yend, img_xlen,
				(color_t*) line_buf[lb_idx]);
